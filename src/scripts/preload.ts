...
import { WizardInterface, KeyableObject } from "../interface";
...
function addEvent(
   ...
   try {
      ...
      el.addEventListener("click", () => {
         if (loading) toggleLoading("accounts");
         event(...params);
      });
   } catch {
      return;
...
function toggleLoading(tab: string, forceClose?: any) {
   let tabEl = document.getElementById(tab);
   if (forceClose) return tabEl.classList.remove("tabLoading");
...
window.addEventListener("DOMContentLoaded", () => {
   function online() {
      document.querySelectorAll(".requiresConnection").forEach((el: HTMLElement) => {
         el.style.display = "";
      });
      document.querySelectorAll(".noConnectionIndicator").forEach((el: HTMLElement) => {
   ...
   fs.readdir(path.join(mainDir, "style", "themes"), (err: any, files: Array<string>) => {
...
let toggledSubTabs: Array<any> = [];
let toggledTabs: Array<any> = [];
...
function drawVersions() {
   ...
   versions
      ...
      .forEach((version: any) => {
         ...
         const formattedDate = new Date(
            version.actualReleaseTime || version.releaseTime,
         ).toShortFormat();
   ...
   vers.innerHTML = els.map((el: HTMLElement) => el.outerHTML).join("");
...
function createAccountList(accounts: any, selected: any) {
   try {
      let tbs: any = [];
      ...
      accounts.forEach((account: any) => {
         ...
         accountImageEl.setAttribute(
            "src",
            `https://visage.surgeplay.com/face/44/${account.profile.id}`,
         );
...
function setSelectedAccount(account: any) {
   try {
      toggleLoading("accounts", true);
      let selectedAccount: KeyableObject = {};
...
function toggleTab(tabName: any, stayOnCurrent: any) {
   if (!tabName) tabName = toggledTabs[0];
   ...
   if (!toggledTabs[0]) {
      ...
      document.getElementById(tabName).classList.add("visibleTab");
      toggler1 && toggler1.classList.add("toggledButton");
      if (!stayOnCurrent) toggledTabs[0] = tabName;
   }
   if (!stayOnCurrent)
      rpc.setActivity({
...
function toggleSubTab(tabName: any) {
   if (!tabName) tabName = toggledSubTabs[0];
   if (!toggledSubTabs[0]) {
...
let launchOptions: KeyableObject = {};

function createProfileList(profiles: any) {
   console.log(profiles);
   let selected = profiles.find((profile: any) => profile.isSelected);
   if (!selected) {
      if (profiles[0]) {
   ...
   launchOptions = versions.find((version: any) => version.id == selected.version);
      ...
      profiles.forEach((account: any) => {
         ...
         !account.isSelected &&
            addEvent(
               ...
               false,
               "selectProfile",
               account.appearance.name,
            );
         addEvent(
            "id",
            "trash-" + account.acronym,
            ipcRenderer.send,
            false,
            "deleteProfile",
            account.appearance.name,
...
function clearObject(object: any) {
   let temp: KeyableObject = {};
...
function wizardCycle() {
   ...
   let page =
      Math.round(
         (wizardEl.scrollLeft - 17 * Math.floor(wizardEl.scrollLeft / wizardEl.offsetWidth)) /
            wizardEl.offsetWidth,
      ) + 1;
...
ipcRenderer.on("hideUi", (event, arg) => {
   if (!arg) {
      document.body.classList.remove("loaded");
      (document.getElementsByClassName("progress")[0] as HTMLElement).style.opacity = "1";
   } else {
...
ipcRenderer.on("progress", (event, arg) => {
   ...
   document.getElementById("progress").innerText =
      arg instanceof Object
         ? `Downloading ${arg.type} ${Math.round((arg.task / arg.total) * 100)}%`
         : arg;
});
...
ipcRenderer.on("updateAvailable", (event, arg) => {
   (document.getElementsByClassName(
      "progress",
   )[0] as HTMLElement).innerText = `Downloading new version ${arg.version} with size of ${(
      arg.size / 1024
   ).toFixed(1)} KB`;
   setTimeout(() => {
...
ipcRenderer.on("updateProgress", (_event, arg) => {
   (document.getElementsByClassName("progress")[0] as HTMLElement).innerText = arg;
});
