         ...
         return new Promise(async (resolve, reject) => {
            ...
            if (fs.existsSync(javaPath)) {
               win.webContents.send(
                  "progress",
                  "Required Java is already installed, skipping installation",
               );
                  ...
                  .then(async (java) => {
                     try {
                        ...
                        const files = Object.keys(res.data.files).map((file) => {
                           return {
                              name: file,
                              downloads: res.data.files[file].downloads,
                              type: res.data.files[file].type,
                           };
                        });
                        ...
                        for (let file of filesToDownload) {
                           ...
                           let download = await axios.get(file.downloads["raw"].url, {
                              responseType: "stream",
                              timeout: 2147483647,
      ...
      async launch(arg) {
        ...
        */
            ...
            try {
               ...
               win.webContents.send(
                  "progress",
                  "Could not download java, skipping automatic java download",
               );
            ...
            if (arg.actualVersion) version.custom = arg.id;
