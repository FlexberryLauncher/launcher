...
async function setup() {
   ...
   if (selectedAccount && !msmc.validate(selectedAccount.profile)) {
      console.log(
         "[MANAGERS => ACCOUNT MANAGER] Deselecting selected account, because it's expired.",
      );
   ...
   msmc
      ...
      .then(async (result) => {
         ...
         if (await db.get("accounts").find({ uuid: result.profile.id }).value()) {
            ...
            event.reply(
               "loginResult",
               JSON.stringify({ status: "error", error: "Account already exists" }),
            );
         } else {
            ...
            event.reply(
               "loginResult",
               JSON.stringify({ status: "success", accounts: JSON.stringify(accs) }),
            );
         }
      })
      .catch((reason) => {
...
ipcMain.on("refreshAccount", async (event, uuid) => {
   ...
   msmc
      ...
      .then(async (result) => {
         ...
         return event.reply(
            "refreshAccountResult",
            JSON.stringify({ status: "success", valid: false, uuid, accounts }),
         );
      })
      .catch((reason) => {
         return event.reply(
            "refreshAccountResult",
            JSON.stringify({ status: "error", error: reason }),
         );
      });
});
...
ipcMain.on("deleteAccount", async (event, data) => {
   ...
   if ((await db.get("accounts").find({ uuid: data }).value()).isSelected) {
      ...
      if (accs[0]) await db.get("accounts").first().assign({ isSelected: true }).write();
      else {
